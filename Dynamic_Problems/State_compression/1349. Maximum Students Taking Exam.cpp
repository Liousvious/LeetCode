...
public:
    ...
    int maxStudents(vector<vector<char>>& seats) {
        ...
        memo = vector<vector<int>> (m + 1, vector<int>(1 << n, 0));
        for (int state = 0; state < (1 << n); state++) {
            int dp = memorableDfs(seats, m, state);
            res = max(res, dp);
        }
        return res;
    }

    int memorableDfs(vector<vector<char>>& seats, int rows, int i) {
        if (rows == 0) return 0;

        if (memo[rows][i] != 0) return memo[rows][i];

        bitset<8> bs(i);
        bool flag = true;
        for (int j = 0; j < n; j++) {
            if ((bs[j] && seats[rows - 1][j] == '#') || (j < n - 1 && bs[j] && bs[j + 1])) {
                flag = false;
                break;
            }
        }
        ...
        for (int k = 0; k < (1 << n); k++) {
            if (memo[rows - 1][k] == -1) continue;
            bitset<8> lbs(k);
            bool flag_1 = true;
            for (int l = 0; l < n; l++) {
                if (lbs[l] && ((l > 0 && bs[l - 1]) || (l < n - 1 && bs[l + 1]))) {
                    flag_1 = false;
                    break;
                }
            }
            if (flag_1) {
                memo[rows][i] = max(memo[rows][i], memorableDfs(seats, rows - 1, k) + (int)bs.count());
            }
        }
        return memo[rows][i] ;
    }
